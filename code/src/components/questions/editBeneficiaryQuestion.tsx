...
class Main extends React.Component<Props, {}> {
  ...
  reRecursiveGetQuestions1(data, questionsList, preQ, actualQuestionLists, isPrefixGroup, prefixGroupCounter) {
    ...
    if (!isEmpty(data)) {
      ...
      for(var i=0; i<(data.length); i++) {
        ...
        /*if ((q.answerState == "valid" || this.questionsAlreadySubmitted(q)) && q.answerState!="invalid" &&  q.answerState!="missing") {
          if (q.hasReflexive) {
            if (q.questions) {
              var reflexsiveQuestionList;
              if (q.type == "group" && prefixGroupCounter==1) {
                reflexsiveQuestionList = this.reRecursiveGetQuestions1(q.questions, questionsList, preQ, actualQuestionLists, isPrefixGroup, prefixGroupCounter +1);
              } else {
                reflexsiveQuestionList = this.reRecursiveGetQuestions1(q.questions, questionsList, preQ, actualQuestionLists, currentIsPrefixGroup, prefixGroupCounter + 1);
              }
      ...
      if (this.questions.data.questionnaire && this.questions.data.questionnaire.questions) {
        ...
        for(var i=0; i<(this.questions.data.questionnaire.questions.length); i++) {
          ...
          /*if ((q.answerState == "valid" || this.questionsAlreadySubmitted(q)) && q.answerState!="invalid" &&  q.answerState!="missing") {
            if (q.hasReflexive) {
              if (q.questions){
                var reflexsiveQuestionList = this.reRecursiveGetQuestions1(q.questions, questionsList, preQ, actualQuestionLists, isPrefixGroup, 2);
                if(reflexsiveQuestionList.length > 0){
                  questionsList.groupHeader = questionsList.groupHeader || [];
                  questionsList.groupHeader.push(qe.caption);
                  return reflexsiveQuestionList;
  ...
  findQuestionById(actualQuestions, questionId, isPrefixGroup, prefixGroupCounter) : any {
